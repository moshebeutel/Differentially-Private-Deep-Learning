...
class GEP(nn.Module):
        ...
        with torch.no_grad():
            ...
            for i, num_param in enumerate(num_param_list):
                ...
                assert pub_grad.shape == (self.num_anchor_grads, num_param), (f'pub_grad.shape: {pub_grad.shape},'
                                                                              f' Expected (self.num_anchor_grads, num_param) {(self.num_anchor_grads, num_param)}')
                pub_grad_centered = pub_grad - centers[i]
                assert pub_grad_centered.shape == pub_grad.shape, (f'pub_grad_centered.shape: {pub_grad_centered.shape},'
    ...
    def forward(self, target_grad, logging=True):
        ...
        for i, num_param in enumerate(num_param_list):
            ...
            assert selected_bases.shape == (num_param, self.num_bases_list[i]), (f'selected_bases.shape: {selected_bases.shape},')
            ...
            assert embedding.shape == (self.batch_size, self.num_bases_list[i]), (f'embedding.shape: {embedding.shape},')
            ...
            if logging:
                ...
                print('group %d, param: %d, num of bases: %d, group wise approx error: %.2f%%'%(i, num_param, self.num_bases_list[i], 100*float(cur_error)))
        ...
        assert avg_clipped_embedding.shape == (sum(self.num_bases_list),), f'avg_clipped_embedding.shape: {avg_clipped_embedding.shape},'
        ...
        if logging:
            with torch.no_grad():
                ...
                print('average norm of residual gradients: ', torch.mean(norms).item(), 'max norm: ', torch.max(norms).item(), 'median norm: ', torch.median(norms).item())

                cosine, angle_deg = cosine_similarity(residual_gradients, no_reduction_approx)
                print('cosine similarity between reconstructed and residual gradients: ', float(cosine), 'angle(deg)', angle_deg)
        ...
        if logging:
            with torch.no_grad():
                ...
                print('cosine similarity between reconstructed and clipped residual gradients: ', float(cosine), 'angle(deg)', angle_deg)
